def solve_inequality(func):
    answer = ''
    prev = None
    for x in range(-100, 100):
        try:
            analize = [func(x - 0.0000001), func(x), func(x + 0.0000001)]
            if not type(analize[0]) == bool:
                print('Error: function must return bool')
                return
        except ZeroDivisionError:
            analize = [func(x - 0.0000001), False, func(x + 0.0000001)]
        if analize[1] and x == 99:
            answer += '+inf)u'
        if analize[1] and x == -99:
            answer += '(-inf,'
...
def find_roots_of_polynomial(a=None, b=None, c=None, d=None):
    '''
